...
def create_dataloader(config, device):
    ...
    # print(config)
    dataframes = []
    # config.data_path = "data/T1DM/csv/1000_0_20210730out_put.csv"

    for index in range(1000, 1013):  # 这个循环会执行多次
        file_pattern = os.path.join("data/T1DM/csv/", f"{index}*.csv")
...
def train(model, args, scaler, device):
    ...
    for i in tqdm(range(epochs)):
        ...
        torch.save(model.state_dict(), "T1DM_final_model/save_model.pth")
        time.sleep(0.1)
...
def valid(model, args, scaler, valid_loader):
    lstm_model = model
    # 加载模型进行预测
    lstm_model.load_state_dict(torch.load("T1DM_final_model/save_model.pth"))
...
def test(model, args, test_loader, scaler):
    ...
    losss = []
    model = model
    model.load_state_dict(torch.load("T1DM_final_model/save_model.pth"))
    model.eval()  # 评估模式
    results = []
    labels = []
